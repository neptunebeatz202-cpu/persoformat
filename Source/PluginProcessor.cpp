...
//==============================================================================
// FIX: Called once per BLOCK. The glide coeff is block-rate so one call per
// block gives a smooth ~30ms crossfade between vowel positions.
    ...
    float morph, float formantShift, float sampleRate)
{
    ...
    for (int k = 0; k < 3; ++k)
    {
        ...
        float safeFreq = juce::jlimit (20.0f, (float)(sampleRate * 0.49f), currentFreq[k]);
...
void PersoFormantAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    // FIX: glide coeff is block-rate.
    // coeff = exp(-1 / (blocksPerSecond * glideTimeSec))
    double blocksPerSec = sampleRate / (double) juce::jmax (1, samplesPerBlock);
    freqGlideCoeff = (float) std::exp (-1.0 / (blocksPerSec * 0.030));
    ...
    for (int ch = 0; ch < 2; ++ch)
    {
        ...
        morphSmoother[ch].reset (sampleRate, 20.0);
        envFollower[ch].reset (sampleRate);
    }

    // Snap to vowel1 immediately so first block needs no glide
    int v1 = (int) getParam ("vowel1");
    for (int k = 0; k < 3; ++k)
    {
        ...
        targetFreq[k]  = currentFreq[k];
        targetGain[k]  = currentGain[k];
        targetQ[k]     = currentQ[k];
    }

    // Push coefficients to filters immediately
    updateFormantCoeffs (0.0f, 1.0f, sampleRate);
}
                                               ...
                                               juce::MidiBuffer&)
{
    ...
    if (envVal > envThreshLin)
        envMorphMod = juce::jlimit (0.0f, 1.0f,
            (envVal - envThreshLin) / (1.0f - envThreshLin + 1e-6f));

    // --- Compute final morph and update filter coefficients ONCE per block ---
    ...
    // --- Sample loop: apply drive then formant filters ---
    for (int s = 0; s < numSamples; ++s)
    {
        for (int ch = 0; ch < numChannels; ++ch)
        {
            ...
            // Drive: soft-clip to add harmonics (helps vowel character come through).
            // Output of softClip is bounded near ±1 so no extra gain compensation needed.
            if (drive > 1.001f)
                x = softClip (x, drive);

            // Three serial peaking-EQ stages shaping the three formant bands
            for (int k = 0; k < 3; ++k)
