...
// THE NEW VISUALISER — full frequency-response curve
{
    ...
    // Vertical frequency grid
    {
        ...
        for (float f : { 100.f, 200.f, 500.f, 1000.f, 2000.f, 3000.f })
        {
            ...
            g.drawVerticalLine((int)x, VY, VY + VH);
            ...
            g.drawText(label,
                       (int)x - 12, (int)(VY + VH - 14), 24, 12,
                       juce::Justification::centred);
        }
    }

    // Fetch live formant state
    ...
    for (int k = 0; k < 3; ++k)
    {
        ...
        if (freq[k] < 10.f)
        {
            freq[k] = kVowelFormants[0].freq[k];
            gain[k] = kVowelFormants[0].gain[k];
            q[k]    = kVowelFormants[0].q[k];
        }
    }
    ...
    {
        juce::Path blob;
        float singleFreq[3] = { 0, 0, 0 };
        float singleGain[3] = { 0, 0, 0 };
        float singleQ[3]    = { 1, 1, 1 };
        ...
        for (int xi = 0; xi <= NX; ++xi)
        {
            ...
            if (xi == 0) blob.startNewSubPath(VX, y);
            else         blob.lineTo(VX + xi, y);
        }

        blob.lineTo(VX + NX, midY);
        blob.lineTo(VX, midY);
        blob.closeSubPath();
        ...
        for (int xi = 0; xi <= NX; ++xi)
        {
            ...
            if (xi == 0) line.startNewSubPath(VX, y);
            else         line.lineTo(VX + xi, y);
        }
    ...
    std::vector<float> ys((size_t)NX + 1);
    ...
    for (int xi = 0; xi <= NX; ++xi)
    {
        ...
        if (xi == 0)
        {
            composite.startNewSubPath(VX, y);
            fillPath.startNewSubPath(VX, y);
        }
        else
        {
            composite.lineTo(VX + xi, y);
            fillPath.lineTo(VX + xi, y);
        }
    }

    fillPath.lineTo(VX + NX, midY);
    fillPath.lineTo(VX, midY);
...

    {
        ...
        g.strokePath(composite, juce::PathStrokeType(3.5f, juce::PathStrokeType::curved,
                                                     juce::PathStrokeType::rounded));

        g.setColour(juce::Colour(0xff40e8ff).withAlpha(0.92f));
        g.strokePath(composite, juce::PathStrokeType(1.5f, juce::PathStrokeType::curved,
                                                     juce::PathStrokeType::rounded));
    }
    ...
    for (int k = 0; k < 3; ++k)
    {
        ...
        g.drawVerticalLine((int)x, VY + VH * 0.08f, VY + VH - 2.f);
        ...
        g.drawText(lbl,
                   (int)(x - 22.f), (int)(VY + 8.f), 44, 22,
                   juce::Justification::centred);

        int idxInt = juce::jlimit(0, NX, (int)(t * NX));
        float peakY = ys[(size_t)idxInt];
    ...
    // Morph scrubber
    {
        ...
        int v1 = juce::jlimit(0, 11,
                              (int)audioProcessor.apvts.getRawParameterValue("vowel1")->load());
        int v2 = juce::jlimit(0, 11,
                              (int)audioProcessor.apvts.getRawParameterValue("vowel2")->load());

        float fMorph = kVowelFormants[v1].freq[1]
                     + morph * (kVowelFormants[v2].freq[1] - kVowelFormants[v1].freq[1]);
        ...
        g.drawText(juce::String((int)(morph * 100.f)) + "%",
                   (int)(x - 14.f), (int)(VY + VH - 24.f), 28, 12,
                   juce::Justification::centred);
    }
